...
The repo consists of a Rust library with simple MacOS (Swift) and Windows (C# WinUI 3) front end applications. For all operating systems which support Rust, there is also a command line version which can be built thus:
...
The app recursively scans the chosen directory. It attempts to retrieve metadata from each supported file type, and uses that data to update the file's `Created` and `Modified` dates.

NB: only the `Created` date is retrieved for video files because `Modified` dates are not available in video metadata. So for videos, the `Created` date is also used to update the `Modified` date as there is usually no significant difference between them anyway. Note also that Linux only supports altering the `Modified` date ([more details here](https://www.figuiere.net/technotes/notes/tn005/)).
